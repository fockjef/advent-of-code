...
function silver(numDim = 3, numIter = 6) {
    let alive = new Set(),
        neigh = getNeigh(numDim);
    parseInput(x => x.split('')).forEach((row, y) => {
        row.forEach((cell, x) => {
            if (cell === '#')
                alive.add(pos2Id([x, y, 0, 0, 0, 0, 0, 0].slice(0, numDim)));
        });
    });
...
function evolve(alive, neigh) {
    ...
    for (let i = 0; i < world.length; i++) {
        let pos = world[i],
            id = pos2Id(pos),
            numNeigh = neigh.filter(offset =>
                alive.has(pos2Id(pos.map((p, i) => p + offset[i])))
            ).length;
...
function pos2Id(pos) {
    let numDim = pos.length,
        ...
        offset = maxPos / 2;
    if (pos.length > 8 || pos.some(p => p < -offset || p >= offset))
        throw new Error('Invalid position');
    return (
        (numDim - 1) * 0x4000000000000 +
        sum(pos.map((p, i) => (p + offset) * Math.pow(maxPos, i)))
    );
}
...
function id2Pos(id) {
    ...
    return Array.from(
        new Array(numDim),
        (p, i) => ((id / Math.pow(maxPos, i)) % maxPos >>> 0) - offset
    );
}

	...
	data.forEach( ( [ cmd, value ] ) => {
		switch( cmd.slice( 0, 3 ) ){
			...
			case "mem":
				...
				mem[addr] = ( ( value / 0x100000000 | orMaskHi ) & andMaskHi ) * 0x100000000 + ( ( ( value | orMaskLo ) & andMaskLo ) >>> 0 );
				break;
		}
	} );
...
function day_14b(){
	...
	data.forEach( ( [ cmd, value ] ) => {
		switch( cmd.slice( 0, 3 ) ){
			...
			case "mem":
				let addr = cmd.slice( 4, -1 ),
				    tmpMask = mask.split( "" ).map( ( bit , i ) => bit === "X" ? bit : ( bit | addr / Math.pow( 2, 35 - i ) % 2 ) ).join( "" ).split( "X" ),

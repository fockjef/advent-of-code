...
function evalNewMath(equation, operators) {
    return evalRPN(
        infixToRPN(tokenizer(equation, operators), operators),
        operators
    );
}
...
function evalRPN(tokens, operators) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        if (tokens[i] in operators) {
            output.push(
                operators[tokens[i]].func(
                    ...output.splice(-operators[tokens[i]].func.length)
                )
            );
        } else {
...
function infixToRPN(tokens, operators) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        if (tokens[i] in operators) {
            ...
            while (
                ops[ops.length - 1] in operators &&
                (operators[ops[ops.length - 1]].precedence > precedence ||
                    (operators[ops[ops.length - 1]].precedence === precedence &&
                        assoc === 'L'))
            ) {
                output.push(ops.pop());
    ...
    return str.match(
        new RegExp(
            [
                '\\d*\\.?\\d+',
                '[)(]',
                ...Object.keys(operators).map(escapeRegExp)
            ].join('|'),
            'g'
        )
    );

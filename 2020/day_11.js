...
function day_11a(){
	function getNeighbors( data, r, c ){
		return [
			...
			.filter( n => n[0] >= 0 && n[0] < data.length && n[1] >= 0 && n[1] < data[n[0]].length && data[n[0]][n[1]] !== "." )
			.map( n => n[0] * data[0].length + n[1] );
	}

	function transitionRule( state, occupied ){
		return ( state === 0 && occupied === 0 ) || ( state === 1 && occupied >= 4 );
	}
...
function day_11b(){
	function getNeighbors( data, r, c ){
		return [
			...
			.map( ( [ dR, dC ] ) => {
				...
				cc = c + dC;
				while( 1 ){
					if( rr < 0 || rr >= data.length || cc < 0 || cc >= data[rr].length ) return null;
	...
	function transitionRule( state, occupied ){
		return ( state === 0 && occupied === 0 ) || ( state === 1 && occupied >= 5 );
	}

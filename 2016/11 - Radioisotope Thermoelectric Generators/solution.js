        ...
        if( !cache.has(id) ){
            ...
            for(let i = 0, f; i < parts.length; i++){
                for(let j = i + 1; j < parts.length; j++){
                    f = moveParts(elevator, elevator + 1, floors, parts[i], parts[j]);
                    if( f ) queue.push({numMoves: numMoves + 1, elevator: elevator + 1, floors: f});
                    f = moveParts(elevator, elevator - 1, floors, parts[i], parts[j]);
                    if( f ) queue.push({numMoves: numMoves + 1, elevator: elevator - 1, floors: f});
                }
...
function parseFloors(){
    ...
    return parseInput()
        .sort((a, b) => rank.indexOf(a.slice(4, 6)) - rank.indexOf(b.slice(4, 6)))
        .map(x => (x.match(/(\w+)(?: generator|-compatible microchip)/g) || []).map(p => {
            let [isotope, type] = p.replace("-compatible", "").split(" ");
            return {isotope, type};
        }));
}

function generateId(elevator, floors){
    return elevator + "|" + floors.length + "|" + Object.values(
        floors
            .map((f, i) => f.map(p => ({isotope: p.isotope, type: p.type, floor: i})))
            .flat()
            .reduce((P, p) => {
    ...
    .sort((a, b) => a.generator - b.generator || a.microchip - b.microchip || a.isotope.localeCompare(b.isotope))
...
function isValidElevator(...parts){
    return parts.length == 1 || (parts.length == 2 && (parts[0].isotope == parts[1].isotope || parts[0].type == parts[1].type));
}

function isValidFloor(...parts){
    let generators = parts.filter(p => p.type == "generator").map(p => p.isotope);
    return generators.length == 0 || parts.filter(p => p.type == "microchip").every(p => generators.includes(p.isotope));
}
...
function decode( patterns, output){
    ...
    digits[1] = patterns.splice( patterns.findIndex( p => bitCount(p) == 2), 1)[0];
    digits[7] = patterns.splice( patterns.findIndex( p => bitCount(p) == 3), 1)[0];
    digits[4] = patterns.splice( patterns.findIndex( p => bitCount(p) == 4), 1)[0];
    digits[8] = patterns.splice( patterns.findIndex( p => bitCount(p) == 7), 1)[0];
    digits[9] = patterns.splice( patterns.findIndex( p => bitCount(p & digits[4]) == 4), 1)[0]; 
    digits[0] = patterns.splice( patterns.findIndex( p => bitCount(p) == 6 && bitCount(p & digits[7]) == 3), 1)[0]; 
    digits[6] = patterns.splice( patterns.findIndex( p => bitCount(p) == 6), 1)[0]; 
    digits[3] = patterns.splice( patterns.findIndex( p => bitCount(p & digits[7]) == 3), 1)[0]; 

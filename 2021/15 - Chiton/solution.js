    ...
    let data = parseInput( x => x.split("").map(Number)),
        cave = function( r, c){
            let offsetR = r / data.length >>> 0,
                ...
                val = data[r % data.length][c % data[0].length] + offsetR + offsetC;
...
function findBestPath( grid, target){
    let maxR = grid.numRows,
        maxC = grid.numCols,
        minCost = Array.from( new Array(maxR), x => new Array(maxC).fill(Infinity)),
    ...
    while( queue.length ){
        ...
        for( let n = 0; n < neighborhood.length; n++ ){
            ...
            rr += r;
            cc += c;
            if( rr >= 0 && cc >= 0 && rr < maxR && cc < maxC && cost + grid( rr, cc) < minCost[rr][cc] ){

...
function silver(t){
    let data = parseInput()[0].match(/-?\d+/g).map(Number),
        ...
        maxStepsX = sumN(minVelX) <= target.x1
            ? Infinity
    ...
    for( let velY = Math.floor((maxStepsX - 1) / 2); velY > 0; velY-- ){
        ...
        for( let i = 0; i < steps.length; i++ ){
            if( steps[i] <= maxStepsX ){
                ...
                if( Math.ceil((target.x0 - posX) / steps[i]) <= Math.floor((target.x1 - posX) / steps[i]) ){
                    return sumN(velY);
                }
            }
        }
...
function gold(t){
    ...
    for( let velY = target.y0; velY < -target.y0; velY++ ){
        ...
        if( stepsY.length ){
            stepsX.forEach( sX => {
                if( stepsY.some( sY => sX.includes(sY) || (sX[1] == Infinity && sX[0] < sY)) ){
                    numInitVel++;
                }
            });
        }
...
function stepsToTargetX( target, v){
    ...
    if( maxPosX < target.x1 ) return [v - Math.floor(invertSumN(maxPosX - target.x0)), Infinity];

...
function gold() {
    ...
    let minRow = Math.min(...dots.map(d => d[1])),
        ...
        sheet = Array.from(new Array(maxRow - minRow + 1), x =>
            new Array(maxCol - minCol + 1).fill('0')
        ),
        code = '';
...
function makeFold(fold, dots) {
    ...
    if (foldAxis == 'x') {
        ...
        foldLine -= minX;
        dots.filter(d => d[0] > foldLine).forEach(
            d => (d[0] -= 2 * (d[0] - foldLine))
        );
    } else {
        let minY = Math.min(0, ...dots.map(d => d[1]));
        foldLine -= minY;
        dots.filter(d => d[1] > foldLine).forEach(
            d => (d[1] -= 2 * (d[1] - foldLine))
        );
    }
}

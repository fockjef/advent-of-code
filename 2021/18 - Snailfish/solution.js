...
function gold(){
    ...
    for( let i = 0; i < data.length; i++ ){
        for( let j = 0; j < data.length; j++ ){
            if( j != i ){
                let magnitude = new SnailfishNumber(data[i]).add(data[j]).reduce().magnitude();
...
const SnailfishNumber = (function(){
    ...
    function explodeAll(nodes){
        while( 1 ){
            let i = nodes.findIndex( ( n, i) => n.depth > 4 && nodes[i+1].depth == n.depth);
            if( i == -1 ) return;
    ...
    function magnitude(nodes){
        while( 1 ){
            ...
            if( maxDepth == 1 ) return 3 * nodes[0].value + 2 * nodes[nodes.length-1].value;

...
function gold(){
    ...
    return Math.max(...[].concat(...scanners.map( ( s1, i) => scanners.slice(i + 1).map( s2 =>
...
function normalizeScanner( refScanner, scanner){
    let scannerAxes = [ 0, 1, 2].map( axis => scanner.map( xyz => xyz[axis])),
        xAxis, yAxis, zAxis;
    xAxis = findCommonAxis( refScanner.map( xyz => xyz[0]), scannerAxes);
    if( !xAxis ) return undefined;
    yAxis = findCommonAxis( refScanner.map( xyz => xyz[1]), scannerAxes);
    if( !yAxis ) return undefined;
    zAxis = findCommonAxis( refScanner.map( xyz => xyz[2]), scannerAxes);
